...
//    var ug = document.getElementById("ug");
    ...
    diszipIndex = ftyp.selectedIndex; // 0-3
    sex = fsex.value;
    sexIndex = fsex.selectedIndex; // 0-1
...
/* weiblich+mänlich: bronze, silber, gold */
    ...
    const soll = Array(3);
    const spAbzOut = Array(4);

    let ofs = 0;
    let result;

    spAbzOut[0] = document.querySelector("#sollbz");
    spAbzOut[1] = document.querySelector("#sollsb");
    spAbzOut[2] = document.querySelector("#sollgd");
    spAbzOut[3] = document.querySelector("#leistung");
    ...
    if (sexidx == 1) { ofs = 3; }
       ...
       for (let i = 0; i < 3; i++) {
           ...
           spAbzOut[i].textContent = soll[i];
       }
       if (leistung > 0) {
           result = sportAbzeichen(leistung, soll, typidx);
           leistung = leistung.toString();
           if ( (typidx >= 2) && (leistung.indexOf('.') <0 ) ) {
             leistung = leistung+".0";
           }
           spAbzOut[3].textContent = leistung;
           setStyle(result);
       }
}
function resetSpAbz(outLine) {
    for(let i = 0;i<4;i++) {
       outLine[i].textContent ="";
    }
    resetStyle();
}
...
function sportAbzeichen(ist, soll, tidx) {
    let res = 0;
    let i;
    if ( tidx < 2 ) {   // Anzahl
       for (i =0; i<3; i++) {
